# Passo F3 — Frontend: Wizard de Onboarding (3 Passos)

## Contexto
Painel em `packages/dashboard/`. Após o registro, novo cliente passa pelo wizard: Persona → WhatsApp → Integrações. Ao final, agente está funcional.

**Pré-requisito:** Passo F2 concluído. Componente de QR Code do Sprint C reutilizado aqui.

## O que Fazer

**1. Crie `src/pages/Onboarding.tsx`**
...
export function Onboarding() {
  ...
  async function handleConnectWhatsApp() {
    ...
    const poll = setInterval(async () => {
      ...
      const status = await axios.get('/api/instances/status');
    ...
    <div style={{ minHeight: '100vh', background: 'var(--paper-2)',
      ...
      <div style={{ width: 520 }}>
        ...
        <div style={{ background: 'var(--paper)', border: '1px solid var(--line)',
          ...
          {step === 1 && (
            <>
              <div style={{ fontFamily: "'Fraunces', serif", fontSize: 26, color: 'var(--ink-1)', marginBottom: 6 }}>
                Como seu agente se comporta?
              </div>
              <div style={{ fontSize: 14, color: 'var(--ink-4)', marginBottom: 24 }}>
                Selecione os tons de voz. Você pode mudar depois.
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 28 }}>
                {TONES.map(t => (
                  <button key={t} onClick={() => setTone(prev =>
              ...
              <button onClick={async () => {
                ...
                setStep(2);
              }} style={{ width: '100%', padding: '11px', borderRadius: 8, fontSize: 14, fontWeight: 500,
          ...
          {step === 2 && (
            <>
              ...
              <div style={{ fontSize: 14, color: 'var(--ink-4)', marginBottom: 24 }}>
                Escaneie o QR Code com o WhatsApp do número que o agente vai usar.
              </div>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                {whatsStatus === 'open' ? (
                  <div style={{ padding: '32px 0' }}>
                    <div style={{ fontSize: 48 }}>✅</div>
                    <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--accent)', marginTop: 12 }}>
                      WhatsApp conectado!
                    </div>
                  </div>
                ) : qr ? (
                  ...
                  <button onClick={handleConnectWhatsApp} style={{
                    padding: '14px 28px', borderRadius: 10, fontSize: 14, fontWeight: 500,
                    border: '1px solid var(--accent-ink)', background: 'var(--accent)',
                    color: '#fff', cursor: 'pointer', fontFamily: 'inherit',
                  }}>
                    Gerar QR Code
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                {whatsStatus === 'open' && (
                  <button onClick={() => setStep(3)} style={{
                    flex: 2, padding: '10px', borderRadius: 8, fontSize: 14, fontWeight: 500,
                    border: '1px solid var(--accent-ink)', background: 'var(--accent)',
                    color: '#fff', cursor: 'pointer', fontFamily: 'inherit',
                  }}>
                    Próximo →
          ...
          {step === 3 && (
            <>
              ...
              <div style={{ fontSize: 14, color: 'var(--ink-4)', marginBottom: 24 }}>
                Configure Asaas e Google Calendar para cobrança e agendamento automáticos. Pode ser feito depois.
              </div>
              {[
                { name: 'Asaas', desc: 'Cobranças e pagamentos automáticos', icon: '💳' },
                { name: 'Google Calendar', desc: 'Agendamento de aulas e sessões', icon: '📅' },
              ].map(i => (
                <div key={i.name} style={{ padding: '14px 16px', background: 'var(--paper-2)',
                  border: '1px solid var(--line)', borderRadius: 10, marginBottom: 10,
              ...
              <button onClick={() => navigate('/dashboard')} style={{
                ...
                fontSize: 14, fontWeight: 600, border: '1px solid var(--accent-ink)',
...
**2. Adicione a rota em `App.tsx`** (rota pública — não precisa de auth, o token já está salvo)
```tsx
<Route path="/onboarding" element={<Onboarding />} />
```

## Verificação
1. Após registrar, wizard abre automaticamente
2. Passo 1: selecionar tons → salva na persona
3. Passo 2: gerar QR → escanear → status muda para "conectado"
4. Passo 3: pular integrações → cai no Dashboard com agente pronto
