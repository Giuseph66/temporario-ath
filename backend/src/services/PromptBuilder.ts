...
function buildRagBlock(context: string): string {
  return `
  <knowledge_context>
    As informações abaixo foram recuperadas da base de conhecimento do agente e são diretamente relevantes para a pergunta atual do usuário. Use-as para embasar sua resposta com precisão. Não as contradiga nem invente informações além delas.
    ---
    ${context}
    ---
...
export function buildSystemPrompt(ctx: PromptContext): string {
  const { state, config, userProfile, ragContext } = ctx;
  ...
  const ragBlock = ragContext ? buildRagBlock(ragContext) : '';

  const blocks = [identityBlock, toneBlock, profileBlock, thinkingBlock, ragBlock, schedulingBlock, stateBlock]
    .filter(Boolean)
    .join('\n  ');
