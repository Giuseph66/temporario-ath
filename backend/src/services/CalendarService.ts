...
export class CalendarService {
    ...
    constructor(calendarId?: string) {
        this.calendarId = calendarId || process.env.GOOGLE_CALENDAR_ID || '';
        if (!this.calendarId) {
            console.warn('⚠️  [CalendarService] GOOGLE_CALENDAR_ID não configurado. Agendamentos não funcionarão.');
        }
    ...
    private get cid(): string {
        if (!this.calendarId) throw new Error('Google Calendar ID não configurado para este tenant.');
    ...
    async checkAvailability(timeMin: string, timeMax: string) {
        try {
            ...
            const busySlots = response.data.calendars?.[this.cid]?.busy || [];
            return busySlots;
        } catch (error) {
