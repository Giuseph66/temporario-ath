...
export async function updatePersona(req: AuthRequest, res: Response): Promise<Response> {
    const { personaJson } = req.body as { personaJson: Record<string, unknown> };
    ...
    // Deep merge — preserva campos do personaJson existente, aplica overrides
    const existing = (agent.personaJson as Record<string, unknown>) ?? {};
    const merged = {
        ...existing,
        ...personaJson,
        tone: { ...(existing.tone as object ?? {}), ...((personaJson.tone as object) ?? {}) },
        protocols: { ...(existing.protocols as object ?? {}), ...((personaJson.protocols as object) ?? {}) },
    };

    await prisma.agent.update({
        where: { id: agent.id },
        data: { personaJson: merged, ...(personaJson.geminiModel ? { geminiModel: String(personaJson.geminiModel) } : {}) },
    });
